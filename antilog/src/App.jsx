        ...
        {user ? (
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
              <input value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleInvite()} placeholder="their@email.com"
              ...
              <button onClick={handleInvite} style={{ ...sans, fontSize: 14, background: C.text, color: "#fff", padding: "9px 16px", borderRadius: 9, cursor: "pointer", border: "none", flexShrink: 0 }}>
                {sent ? "Sent ✓" : "Send"}
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
              <div style={{ ...sans, fontSize: 14, flex: 1, border: `1px solid ${C.border}`, borderRadius: 9, padding: "9px 12px", color: C.textSub, background: C.card, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{link}</div>
              <button onClick={copyLink} style={{ ...sans, fontSize: 14, background: copied ? "#edf7ed" : "#f0ebe3", color: copied ? "#2e6a2e" : C.textMid, padding: "9px 14px", borderRadius: 9, cursor: "pointer", border: "none", flexShrink: 0, transition: "all 0.2s" }}>
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
            <button onClick={signOut} style={{ ...sans, fontSize: 13, color: C.textMuted, width: "100%", background: "none", border: `1px solid ${C.border}`, borderRadius: 9, padding: "8px", cursor: "pointer" }}>Sign out</button>
          </>
        ) : (
          <>
...
function ShareSheet({ text, author, onClose }) {
  ...
  const channels = [
    ...
    { name: "LinkedIn",    icon: "in", url: `https://www.linkedin.com/sharing/share-offsite/?url=https://antilog.peoplebones.com&summary=${enc}` },
  ...
  return (
    <div style={{ marginTop: 10, paddingLeft: 12, borderLeft: `2px dashed ${accentColor}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
        ...
        <button onClick={onScrollToCard} style={{ ...sans, fontSize: 11, color: C.textMuted, background: "none", border: "none", cursor: "pointer", padding: 0, textDecoration: "underline", textDecorationStyle: "dotted" }}>
...
// ── Rebuttal full card ────────────────────────────────────────────────────
function RebuttalCard({ rebuttal, originalArg, onScrollToOriginal, onShare, onFork, onOpenProfile, onRep, cardRef, isAdmin, onDelete, onFactCheck }) {
  const accentColor = rebuttal.rebuttalSide === "pro" ? C.pro : C.con;
  ...
  return (
    <div ref={cardRef} style={{ borderLeft: `3px solid ${accentColor}`, background: C.card, borderRadius: "0 12px 12px 0", padding: "14px 14px 12px" }}>
      <button onClick={onScrollToOriginal}
        style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, padding: "6px 10px", background: "#f2ede6", borderRadius: 8, width: "100%", textAlign: "left", cursor: "pointer", border: "none", borderLeft: `2px solid ${originalAccent}` }}>
...
// ── Arg card ──────────────────────────────────────────────────────────────
function ArgCard({ arg, side, rebuttals, cardRef, onFork, onShare, onOpenProfile, onRep, onAddRebuttal, onScrollToRebuttalCard, user, isAdmin, onSignIn, onDelete, onFactCheck }) {
  ...
  return (
    <div ref={cardRef} style={{ borderLeft: `3px solid ${accent}`, background: C.card, borderRadius: "0 12px 12px 0", padding: "14px 14px 12px" }}>
      ...
      {replies.map(r => <ReplyCard key={r.id} reply={r} onFork={onFork} onShare={onShare} onOpenProfile={onOpenProfile} onRep={onRep} onFactCheck={onFactCheck} />)}
      {isAdmin && (
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 4 }}>
          <button onClick={() => { if (window.confirm("Delete this argument and all its replies?")) onDelete("argument", arg.id); }}
            style={{ ...sans, fontSize: 12, color: "#c0504d", background: "none", border: "1px solid #e8c8c8", borderRadius: 6, padding: "2px 10px", cursor: "pointer" }}>🗑</button>
        </div>
      )}
      {showReplyForm && (
      ...
      {argRebuttals.map(r => (
        <RebuttalPreview key={r.id} rebuttal={r} onScrollToCard={() => onScrollToRebuttalCard(r.id)} onOpenProfile={onOpenProfile} onRep={onRep} />
      ))}
    </div>
  );
}
...
function MobileTabs({ args, rebuttals, argRefs, rebuttalCardRefs, pro, con,
  ...
  user, isAdmin, onSignIn, onDelete, onFactCheck }) {
    ...
    <div>
        ...
        {[["pro", "For", C.pro, proW], ["con", "Against", C.con, conW]].map(([key, label, color, pct]) => {
          ...
          return (
            <button key={key} onClick={() => setTab(key)}
              style={{ flex: 1, padding: "10px 8px", border: "none", cursor: "pointer", background: active ? color : "#faf8f5", transition: "background 0.18s", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
      ...
      {tab === "pro" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {args.pro.map(a => (
            <ArgCard key={a.id} arg={a} side="pro" rebuttals={rebuttals}
              ...
              onFork={onFork} onShare={onShare} onOpenProfile={onOpenProfile} onRep={onRep}
              onAddRebuttal={onAddRebuttal} onScrollToRebuttalCard={onScrollToRebuttalCard}
              user={user} isAdmin={isAdmin} onSignIn={onSignIn} onDelete={onDelete} onFactCheck={onFactCheck} />
          ))}
          {proRebuttalCards.map(r => (
            <RebuttalCard key={r.id} rebuttal={r} originalArg={findArg(r.argId)}
              ...
              onShare={onShare} onFork={onFork} onOpenProfile={onOpenProfile} onRep={onRep}
              isAdmin={isAdmin} onDelete={onDelete} onFactCheck={onFactCheck} />
          ))}
          <AddArgPanel side="pro" onAdd={arg => onAddArg("pro", arg)} user={user} onSignIn={onSignIn} />
          <button onClick={() => setTab("con")} style={{ ...sans, fontSize: 14, color: C.con, background: "none", border: `1px solid ${C.con}`, borderRadius: 10, padding: "10px", cursor: "pointer", marginTop: 8, opacity: 0.85 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <button onClick={() => setTab("pro")} style={{ ...sans, fontSize: 14, color: C.pro, background: "none", border: `1px solid ${C.pro}`, borderRadius: 10, padding: "10px", cursor: "pointer", marginTop: 8, opacity: 0.85 }}>
            ← See the case For
          </button>
        </div>
      )}
...
function DebateView({ topic, onBack, onArgUpdate, onForkCreated, onOpenProfile, onRep, user, isAdmin, onDelete, onSignIn }) {
  ...
  const registerFactCheck = (argId, result) => {
    setFactChecks(prev => ({ ...prev, [argId]: result }));
  };
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {args.pro.map(a => (
                <ArgCard key={a.id} arg={a} side="pro" rebuttals={rebuttals}
                  ...
                  onFork={setForkTarget} onShare={setShareTarget}
                  onOpenProfile={onOpenProfile} onRep={onRep}
                  onAddRebuttal={addRebuttal} onScrollToRebuttalCard={scrollToRebuttalCard}
                  user={user} isAdmin={isAdmin} onSignIn={onSignIn} onDelete={onDelete} onFactCheck={registerFactCheck} />
              ))}
              {proRebuttalCards.map(r => (
                <RebuttalCard key={r.id} rebuttal={r} originalArg={findArg(r.argId)}
                  cardRef={el => { rebuttalCardRefs.current[r.id] = el; }}
                  ...
                  onOpenProfile={onOpenProfile} onRep={onRep}
                  isAdmin={isAdmin} onDelete={onDelete} onFactCheck={registerFactCheck} />
              ))}
            </div>
...
function TopicCard({ topic, onClick, isAdmin, onDelete }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {isAdmin && (
        <button onClick={e => { e.stopPropagation(); if (window.confirm("Delete this truth and all its arguments?")) onDelete("topic", topic.id); }}
    ...
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "36px 20px" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          <button onClick={() => user ? setShowForm(true) : onSignIn()} style={{ ...sans, fontSize: 14, background: C.text, color: "#fff", padding: "9px 18px", borderRadius: 10, cursor: "pointer", border: "none" }}>
            {user ? "+ New Truth" : "Sign in to post"}
          </button>
        </div>
      </div>
      ...
      {mobile ? (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
            ...
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#e8e0d4", color: C.textSub, display: "flex", alignItems: "center", justifyContent: "center", ...sansMed, fontSize: 13 }}>
              {user ? (user.user_metadata?.full_name?.[0] ?? "U") : SESSION_USER.avatar}
            </div>
          </div>
        </div>
      ) : (
...
export default function App() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const shaped = data.map(t => ({
            ...
            hot: t.hot, forkedFrom: t.forked_from, forkedAuthor: t.forked_author,
            ...
            args: {
              pro: (t.arguments || []).filter(a => a.side === "pro" && !a.parent_id).map(a => ({
                id: a.id, author: a.author_name, votes: a.votes, text: a.text, source: a.source,
                replies: (t.arguments || []).filter(r => r.parent_id === a.id).map(r => ({ id: r.id, author: r.author_name, votes: r.votes, text: r.text, source: r.source })),
              })),
              con: (t.arguments || []).filter(a => a.side === "con" && !a.parent_id).map(a => ({
                id: a.id, author: a.author_name, votes: a.votes, text: a.text, source: a.source,
                replies: (t.arguments || []).filter(r => r.parent_id === a.id).map(r => ({ id: r.id, author: r.author_name, votes: r.votes, text: r.text, source: r.source })),
              })),
            },
  ...
  const addTopic = useCallback(async (t) => {
    ...
    try {
      ...
      const { data, error } = await c.from("topics").insert({
        title: t.title, summary: t.summary, category: t.category, hot: false,
        created_by: user?.id ?? null, forked_from: t.forkedFrom ?? null, forked_author: t.forkedAuthor ?? null,
      }).select().single();
  ...
  const sessionRep = user ? (USERS[user.user_metadata?.full_name]?.rep ?? 0) : SESSION_USER.rep;
  const ADMIN_EMAILS = ["vinoly@gmail.com"];
  const isAdmin = user && ADMIN_EMAILS.includes(user.email);

  const handleDelete = useCallback(async (type, id) => {
    if (type === "topic") setTopics(ts => ts.filter(t => t.id !== id));
    try {
